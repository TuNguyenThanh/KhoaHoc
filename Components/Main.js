...
export default class Main extends Component {
  constructor(props){
    ...
    this.state = {
      ...
      dataSourceTop: new ListView.DataSource({rowHasChanged:(r1,r2)=>r1!=r2}),
    };
  }
    ...
    return(
      <View style={styles.row}>
        <View style={styles.headerkhoahoc}>
          ...
          <TouchableOpacity onPress={this.props.seeAll.bind(this, [item.id, item.name])}>
        ...
        <ListView style={styles.lst2}
          horizontal={true}
          dataSource={
            new ListView.DataSource({rowHasChanged:(r1,r2)=>r1!=r2}).cloneWithRows(item.daotao_by_nganh)}
  ...
  renderHeader() {
    return (
      <View style={{width:width, height: height/3, backgroundColor:'white'}}>
        <View style={styles.headerkhoahoc}>
          <Text style={styles.tenkhoahoc}>Top Ngành Hot</Text>
        </View>

        <ListView
          horizontal={true}
          dataSource={this.state.dataSourceTop}
          renderRow={this.createRowTop.bind(this)}
        />
      </View>
    )
  }
  ...
  createRowTop(item){
    return(
      <TouchableOpacity onPress={this.props.itemTopClick.bind(this,[item.id,item.name])}>
        <Image style={styles.rowTop}
          source={{uri: item.image}}
        />
      </TouchableOpacity>
    );
  ...
  componentDidMount(){
    fetch("http://khoahoc.nhodalat.com/topnganhhot")
    .then((response) => response.json())
    .then((responseJson) => {
      mangTop = responseJson;
      this.setState({
        dataSourceTop: this.state.dataSource.cloneWithRows(mangTop)
      });
    })
    .catch((error)=> {
      console.error(error);
    ...
    fetch("http://khoahoc.nhodalat.com/main")
    .then((response) => response.json())
    .then((responseJson) => {
      mang = responseJson;
      console.log(mang);
...
//
//    width: (width - 60)/6 - (Platform.OS === 'ios' ? 0 : 4),
//    height: (width - 60)/6 - (Platform.OS === 'ios' ? 0 : 4)
}
